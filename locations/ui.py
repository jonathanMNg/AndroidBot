from object.coordinate import Coordinate

ui = {
    'switch_city_map_view': Coordinate(33, 615),
    'quest': Coordinate(100, 615),
    'bag': Coordinate(166, 615),
    'mail': Coordinate(233, 615),
    'alliance':  {
        'coordinate': Coordinate(300, 615),
        'functions': {
            'alliance_help': Coordinate(190, 415),
            'help_all': Coordinate(202, 616)
        }
    },
    'my_info': Coordinate(366, 615),
    'building_upgrade_1': Coordinate(25, 105),
    'battle_power_score': Coordinate(45, 65),
    'ui_helper_expand': Coordinate(3, 257),
    'activate_commander_skills':  {
        'coordinate': Coordinate(372, 512),
        'functions': {
            'harvest': Coordinate(56, 517),
            'recall': Coordinate(253, 515),
            'use': Coordinate(315, 305)
        }
    }
}

